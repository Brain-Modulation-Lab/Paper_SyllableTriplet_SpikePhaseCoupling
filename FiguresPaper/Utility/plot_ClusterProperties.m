...
for prop_i = 1 : nProperties
    ...
    switch Property
        case 'Frequency_1DHist'
            ...
            raincloud_plot(ClusterCentroid(:,2),"box_on",1,"color",[.8 .8 .8],"alpha",.3);
            hold on
        ...
        case 'Duration_1DHist'
            ...
            fh{prop_i} = figure('Position',[400 200 600 400]);
            raincloud_plot(ClusterDur,"box_on",1,"color",[.8 .8 .8],"alpha",.3)
            hold on
        ...
        case 'TaskModulation_1DArea'
            ...
            fh{prop_i} =figure("renderer","painters","position",[400 200 600 1600]);
            tiledlayout(2,2)
            nexttile(1,[1 2])
            hold on
...
%             end
            ...
            for fi = 2:6
                nexttile
                plot(T,cluster_distr_bands(fi,:))
                hold on
                plotshaded(T,[squeeze(prctile(cluster_probperm_bands(fi,:,:),5,3)); ...
...
%                 end
            ...
            %              hold on
            %              plot( prctile(cluster_prob_perm,5,2)')
            %              plot( prctile(cluster_prob_perm,50,2)')
            %              plot( prctile(cluster_prob_perm,95,2)')
            %
            %
            %
            ...
            %             for evt_i = 1:5
            %                 subplot(5,1,evt_i)
            %                 raincloud_plot(cluster_prob_perm_task(:,evt)  ,'box_on',1,'color',[.8 .8 .8],'alpha', .3)
            %                 hold on
            %                 scatter(cluster_distr_task(evt_i),0,'k','filled')
            %                 box off
            %
            %             end
            %
            ...
            %                 for evt_i = 1:5
            %                     subplot(5,5,cc)
            %                     raincloud_plot(cluster_prob_perm_bandtask(:,fi,evt_i)*sum(ClusterCentroidBand == fi)/nClusters_all,'box_on',1,'color',[.8 .8 .8],'alpha',.3)
            %                     hold on
            %                     scatter(cluster_distr_bands_task(fi,evt_i),0,'k','filled')
            %                     cc= cc+1;
            %                 end
            %             end
            %
            ...
            %             %
            %             nexttile(3,[1 1])
            %             bar(1 : numel(Task_labels),cluster_distr_task,"FaceColor", [.8 .8 .8], 'FaceAlpha', .4);
            %             xticks(1 : numel(Task_labels))
            ...
            %
            %             nexttile(4,[1 1])
            %             bar(1 : numel(Task_labels), flipud(cluster_distr_bands_task(2:end,:))','stacked');
            %             xticks(1 : numel(Task_labels))
            %             xticklabels(Task_labels)
            %             ylabel(' t-SPC occurrence [a.u.] ')
            %             box off
            %             legend(bands.symbol,'location','best')

            %             for fi = 2:6
            %                 hold on
            %             plot(1 : numel(Task_labels),cluster_distr_bands_task(fi,:), ...
            %                 'linewidth',2.5,'color',hex2rgb(bands.color(fi,:)),'marker','o', ...
            %                 'markerfacecolor',hex2rgb(bands.color(fi,:)))
            %             end

        case 'RobustnessCheck'
            cfg.VarFields = {'ClusterCycles'};

            ClusterCycles = get_SPCClustersProperties(cfg,Clusters);
            fh{prop_i} = figure('position',[200 200 500 500]);
            nexttile
            raincloud_plot(ClusterCycles(isfinite(ClusterCycles)),'box_on', 1 ,...
                'color',[.8 .8 .8],'alpha',.3 ,'density_type','rash')
            hold on

            xline(median(ClusterCycles(isfinite(ClusterCycles))),'r--',sprintf(['   %1.1f ' char(177)  ' %1.1f (%1.2f %%, < 2) '],median(ClusterCycles(isfinite(ClusterCycles))),iqr(ClusterCycles(isfinite(ClusterCycles))), mean(ClusterCycles(isfinite(ClusterCycles)) < 5)*100),'LabelOrientation','horizontal','linewidth',1.5)
            box off
            grid off
            xlabel(" Clusters  # cycles " )
            ylabel(" PDF [a.u.] ")
            %yticks([0 2 4])
            xlim([0 1e+3])
            nexttile

            bootsp_chance = [0.34168 0.296690246157020]/100; % estimated before;
            bootsp_chancesign = bootsp_chance(1) + bootsp_chance(2)*randn(1, 1000);
            if all((bootsp_chancesign*19577) <= 2943  )
                pval_chancesign = 1/1000;
            else
                pval_chancesign = sum(2943 >= (bootsp_chancesign*sum(19577,[],'omitnan')))/(1000 + 1);
            end
            raincloud_plot((bootsp_chancesign*19577),"box_on",1,"color",[.8 .8 .8],"alpha",.3,'density_type','rash')
            hold on
            if pval_chancesign > 0.05
                xline(2943,'r--',sprintf(' p = %1.2f', .05),'LabelOrientation','horizontal','linewidth',1.5)
            elseif pval_chancesign > 0.01 && pval_chancesign <= 0.05
                xline(2943,'r--',sprintf(' p < %1.2f*', .05),'LabelOrientation','horizontal','linewidth',1.5)
            elseif pval_chancesign > 0.001 && pval_chancesign <= 0.01
                xline(2943,'r--',sprintf(' p < %1.2f**', .01),'LabelOrientation','horizontal','linewidth',1.5)
            else
                xline(2943,'r--',sprintf(' p < %1.3f***', .001),'LabelOrientation','horizontal','linewidth',1.5)
            end
            box off
            grid off
            xlabel(" # Sign. clusters  " )
            ylabel(" PDF [a.u.] ")
            %yticks([0 2 4])
            xlim([0 3e+3])
        case 'TaskModulation_1DPhaseAggregation'
            ...
            for fi = 2:6
                ...
                %         'o','color',[0 0 0 .4],'MarkerSize',4,'LineWidth',0.1,...
