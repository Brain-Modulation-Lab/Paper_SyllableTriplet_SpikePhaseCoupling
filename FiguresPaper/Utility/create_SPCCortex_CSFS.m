...
%     peak = ClusterS_MNI_Band_peak(fi,:);
...
fh{2} = figure('position',[200 200 400 400]);
...
for roi_i = 1 : numel(ROI_AtlasLabels)
    ...
    box off
    hold on
    xline( median(ROI_ClustDur{roi_i}),'r--',sprintf('   %1.2f Hz ', median(ROI_ClustDur{roi_i})),'LabelOrientation','horizontal','linewidth',1.5)
...
% phase
% ROI_ClustPhase = nan(numel(ROI_AtlasLabels),3);
% for roi_i = 1 : numel(ROI_AtlasLabels)
%   [mu ul ll]   = circ_mean(ClusterPhase(strcmpi(ClusterEAtlasLabel,ROI_AtlasLabels{roi_i}),1));
% ROI_ClustPhase(roi_i,:) = [mu ul ll];
% end
% figure('position',[200 200 400 400])
% polarscatter(ROI_ClustPhase(:,1),1:numel(ROI_AtlasLabels),'k','filled')
...
fh{7} = figure('position',[200 200 1200 400]);
tiledlayout(1,4)
for unit_i = 1:4
    nexttile
...
%             ROI_FrequencyBand_SPCdensity_perm{fi,roi_i}(perm_i) = 100*(strcmpi(tmp2,ROI_AtlasLabels{roi_i}))/;
%         end
...
% 
%fh{8} = figure('position',[200 200 1200 600]);

% figure('renderer','painters','position',[300 300 600 600])
% hold on
% ph = nan(1,6);
% for fi = 1:6
%     plot(1:numel(ROI_AtlasLabels),     ROI_FrequencyBand_density(fi,:) ,'linewidth',1.4)
%     ph(fi) = scatter(1:numel(ROI_AtlasLabels),     ROI_FrequencyBand_density(fi,:),75,'filled');
% end
% xticks(1:numel(ROI_AtlasLabels))
% xticklabels(ROI_AtlasLabels)
% ylabel(' # Clusters per pair [a.u.]')
% set(gca,'TickLabelInterpreter','none')
% %legend(ph(2:end),bands.symbol(2:end),'interpreter','tex')
% box off
...
%figures,'group-level','theta_cluster1_clusterness-stat');
...
% tiledlayout(1,3)
% nexttile
% scatter(ClusterS_MNI(:,1),ClusterCentroid(:,2),'k','filled');
% nexttile
% scatter(ClusterS_MNI(:,2),ClusterCentroid(:,2),'k','filled');
% nexttile
% scatter(ClusterS_MNI(:,3),ClusterCentroid(:,2),'k','filled');
